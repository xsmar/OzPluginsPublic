...
@Slf4j
@Singleton
class AutoEnchanterOverlay extends OverlayPanel {
    private final AutoEnchanterPlugin plugin;
    private final AutoEnchanterConfiguration config;
    ...
    @Inject
    private AutoEnchanterOverlay(final Client client, final AutoEnchanterPlugin plugin, final AutoEnchanterConfiguration config) {
        super(plugin);
        setPosition(OverlayPosition.BOTTOM_LEFT);
        this.plugin = plugin;
        this.config = config;
        getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Auto Enchanter overlay"));
    }
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        panelComponent.getChildren().add(TitleComponent.builder()
                .text("Auto Enchanter " + config.version)
